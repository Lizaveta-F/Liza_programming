...
parser.add_argument("--size", default=4,
                    help="Enter the number of rows and columns")
...
def init_state():
    state = dict()
    # matrix 4x4 with initial number "2" in random cell
    state['matrix'] = generate_state()
    state['score'] = 0
    # read best score from some file
    state['best_score'] = load_state(state)['score']
    if not pathlib.Path('state.pkl').exists():
        with open('state.pkl', 'wb') as fh:
            pickle.dump(state, fh)
    return state
...
def run():
    ...
    while True:
        ...
        if e.event_type != 'up':
            continue  # handling keys push up
        cmd = e.name
